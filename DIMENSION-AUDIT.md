...
| 4. 性能与压测 | **9** | 优秀 |
| 5. 安全与合规 | **9** | 优秀 |
| 6. 测试与质量 | **9** | 优秀（Round-5 pytest 扩充 + **Round-6 project-hub-2 复验 ✓**） |
| 7. API 与架构 | **9** | 优秀 |
...
- **Round-6 复验**（project-hub-1 + **project-hub-2** · 2026-07-06）：`python -m pytest --cov=app` → **54 passed** · **80.76%** cov · 门槛 75% 通过。
- **覆盖率明细**：`rag.py` **94%** · `retrieval.py` **92%** · 核心链路 `build_context` / `condense_question` / 向量降级 BM25 均有单测。
...
| ~~P2~~ | ~~深色模式 / 响应式表格 polish~~ ✅ Round-7 · CSS 变量全覆盖 + 表格 hover/scroll |
| P3 | 文档上传进度与解析状态实时推送 |
...
| 6 | **P2** | nightly k6 CI | 性能 9→10 |
| 7 | **P3** | Playwright E2E | 演示 9→10 |
| 8 | **P3** | 复合 DB 索引 + 复测 | 性能巩固 |
...
*Round-6 复测 · 均分 **8.9** · 下一目标：公网 HTTPS Phase-2 证据 + 深色模式 polish → **9.0+**。*
