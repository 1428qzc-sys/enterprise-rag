...
def create_kb(body: KBCreate, session: Session = Depends(get_session)) -> KBRead:
    kb = KnowledgeBase(
        name=body.name,
...
def list_kbs(session: Session = Depends(get_session)) -> List[KBRead]:
    kbs = session.exec(select(KnowledgeBase).order_by(KnowledgeBase.created_at.desc())).all()
    return [_to_read(session, kb) for kb in kbs]


@router.get("/{kb_id}", response_model=KBRead)
def get_kb(kb_id: str, session: Session = Depends(get_session)) -> KBRead:
...
def update_kb(kb_id: str, body: KBUpdate, session: Session = Depends(get_session)) -> KBRead:
    kb = session.get(KnowledgeBase, kb_id)
    if kb is None:
        raise HTTPException(status_code=404, detail="知识库不存在")
...
def delete_kb(kb_id: str, session: Session = Depends(get_session)) -> None:
    kb = session.get(KnowledgeBase, kb_id)
    if kb is None:
        raise HTTPException(status_code=404, detail="知识库不存在")
